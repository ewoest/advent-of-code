...
def get_value_direction(matrix, point, dir):
    cur = point
    while True:
        cur = add_points(cur, dir)

        if not is_valid_point(matrix, cur):
            return None
        
        c = matrix[cur[1]][cur[0]]
...
def count_around_point(matrix, point, target):
    ...
    for dir in all_directions:
        if get_value_direction(matrix, point, dir) == target:
            count = count + 1
