...
all_options_return = [dir_up, dir_left, dir_down, dir_right, wait]
dirs_right_turn = [dir_right, dir_down, dir_left, dir_up]

def is_valid_point(point, matrix):
    return (point[1] < len(matrix) 
            and point[1] >= 0
            and point[0] < len(matrix[0]) 
            and point[0] >= 0
            and matrix[point[1]][point[0]] != '#')
...
def move_blizzards(minute, matrix):
    ...
    prev_blizzards = move_blizzards(minute-1, matrix)
...
def find_path(start, end, start_minute, blizzards, matrix, move_options):
    DF[0] = blizzards

    queue = PriorityQueue()
    queue.put((distance(start, end), distance(start, end), start_minute, start, blizzards))

    num_visited = 0
    num_skipped = 0

    min_minutes = 10000000000
    visited = set()
    ...
    while not queue.empty():
        ...
        for option in move_options:
            ...
            if is_valid_point(option_pos, matrix) and option_pos not in cur_blizzards:
                option_dist = distance(option_pos, end)
                if (option_minute, option_pos) in visited:
                    continue
                if (option_minute + option_dist) > min_minutes:
                    continue

                queue.put(((option_dist), option_dist, option_minute, option_pos, cur_blizzards))
...
def process_file(filename: str):
    ...
    # print(f"Starting 1st path")
    # minutes_first_path = find_path((1,0), destination, 0, blizzards, matrix, all_options)

    # print(f"Starting 2nd path")
    # minutes_second_path = find_path(destination, (1,0), minutes_first_path, blizzards, matrix, all_options_return)

    minutes_second_path = 620

    print(f"Starting 3nd path")
    minutes_third_path = find_path((1,0), destination, minutes_second_path, blizzards, matrix, all_options)
    
    print(f"minutes_third_path: {minutes_third_path}")
