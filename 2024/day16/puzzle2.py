...
def traverse(matrix, start, end, start_dir):
    queue = PriorityQueue()
    queue.put((0, start, start_dir, set()))
    ...
    while not queue.empty():
        (score, point, cur_dir, path) = queue.get()
        ...
        if point == end:
            best_score = score
            all_points = all_points.union(new_path)
            continue
        ...
        if next_value != '#':
            queue.put((score+1, next_point, cur_dir, new_path))
...
def solve(filename):
    ...
    all_points = traverse(matrix, start, end, dir_right)
    score = len(all_points)
    print(f'score: {score}')
