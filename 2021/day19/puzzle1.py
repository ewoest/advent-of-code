        ...
        for j in range(i+1, len(scanners)):
            ...
            for key in dist1:
                if key in dist2:
                    ...
                    # print(f"key = {key}")
                    # print(f"pai1 = {pair1}. pair2 = {pair2}")
                    # print()
            ...
            pair_occurrences = {i:pair_occurrences[i] for i in pair_occurrences if pair_occurrences[i]>=3}

            # print(f"pair_occurences: {pair_occurrences}")
            for pair in pair_occurrences:
                if pair[1] in all_beacons:
                    all_beacons.remove(pair[1])
            # else:
            #     print(f"no overlap between scanners {i} and {j} - {len(pair_occurrences)}")

    print(f"num beacons = {len(all_beacons)}")
