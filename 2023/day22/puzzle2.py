...
def count_blocks_fall(block):
    blocks_fall : set[Block] = set()
    queue = [block]

    while queue:
        current:Block = queue.pop(0)

        blocks_fall.add(current)

        for supp in current.supporting:
            if supp.supported_by.issubset(blocks_fall):
                blocks_fall.add(supp)
                queue.append(supp)
    
    return len(blocks_fall) - 1
...
def main(filename):
    ...
    for block in blocks:
        fall = count_blocks_fall(block)
        print(f"If block {block.label} falls, {fall} other blocks will falls")
        total += fall
